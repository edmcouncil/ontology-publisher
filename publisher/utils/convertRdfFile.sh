...
function convertRdfFileTo() {
  ...
  #	* use "fixed" riot to convert to json-ld in JSONLD10_EXPAND_PRETTY format
 ...
 if [ "${targetFormat}" = "json-ld" ] ; then
  ${JENA_RIOT} --formatted=JSON-LD-10 "${rdfFile}" \
    >  "${targetFile}.tmp" \
    2> "${logfile}"
  rc=$?
 else
...
#   ((verbose)) && logItem "Adjusting ttl base IRI" "$(logFileName "${rdfFile}")"
    ${SED} -i "s?^\(\(# baseURI:\)\|\(@base\)\).*ontology/?&${branch_tag:+${branch_tag}/}?" "${targetFile}"
    test -z "${branch_tag}" || ${SED} -i "s@${branch_tag}/${branch_tag}/@${branch_tag}/@" \
	  "${targetFile}"
  fi
